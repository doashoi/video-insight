...
class VideoAnalyzer:
                ...
                if asr_res and len(asr_res) > 0:
                    ...
                    if clean_text:
                        ...
                        if len(sub_sentences) > 1:
                            ...
                            raw_sentences.append(
                                {"start": start_ms, "end": end_ms, "text": clean_text}
                            )
    ...
    def extract_frames(
        self, video_path: str, anchors: List[float], temp_dir: str
    ...
    def remove_duplicate_frames(
        self, frame_info: List[Tuple[float, str]], threshold: int = 5
    ) -> List[Tuple[float, str]]:
        """移除重复帧。"""
        if not frame_info:
            return []
        ...
        for ts, path in frame_info:
            ...
            if img is not None:
                frame_hashes.append(
                    {"ts": ts, "path": path, "hashes": self._get_hashes(img)}
                )
            else:
        ...
        print(
            f"[Dedup] {len(frame_info)} -> {len(final_list)} (过滤了 {filtered_pairs_count})"
        )
        ...
        with open(report_path, "w", encoding="utf-8") as f:
            ...
            for item in report_data:
                ...
                f.write(
                    f"pHash:{item['distances']['phash']} dHash:{item['distances']['dhash']} "
                )
                f.write(
                    f"Diff:{item['distances']['pixel_diff']:.2f} Keep:{item['kept']}\n"
                )

        return [(f["ts"], f["path"]) for f in final_list]

    def create_contact_sheet(
        self, frame_info: List[Tuple[float, str]], output_base_path: str
    ) -> List[str]:
        """创建拼图 (九宫格)。"""
        if not frame_info:
            return []
...
def process_video_folder(video_folder: Path, output_root: Path, progress_callback=None):
    ...
    video_files = [
        f for f in video_folder.iterdir() if f.suffix.lower() in valid_extensions
    ]
    ...
    if progress_callback:
        progress_callback(
            f"🎵 正在提取音频并进行语音识别，共计 {len(video_files)} 条..."
        )
    ...
    for video_file in video_files:
        ...
        if results:
            with open(transcript_path, "w", encoding="utf-8") as f:
                for item in results:
                    f.write(
                        f"[{item['start'] / 1000:.2f}s - {item['end'] / 1000:.2f}s] {item['text']}\n"
                    )
            audio_success_count += 1
        else:
    ...
    if progress_callback:
        progress_callback(
            f"🖼️ 音频提取完成，正在进行截图，共计 {len(video_files)} 条..."
        )
    ...
    for video_file in video_files:
        ...
        frame_info = analyzer.extract_frames(
            str(video_file), anchors, str(image_out_dir)
        )
